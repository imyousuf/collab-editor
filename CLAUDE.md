...
- **WYSIWYG position mapping**: Y.Text stores raw Markdown/HTML source, but ProseMirror stores the parsed structure without syntax characters (`#`, `**`, `<p>`, `<strong>`, etc.). `frontend/src/collab/pm-position-map.ts` is the shared helper used by both `blame-tiptap-plugin.ts` and `comment-tiptap-plugin.ts` to map Y.Text offsets to PM positions via a substring walker (`yText.indexOf(node.text, cursor)`). Do not reintroduce per-plugin `buildPositionMap` — the duplication was the root cause of historical decoration-drift bugs
- **Formatting authorship**: when one user wraps another user's text in a mark (e.g., User-A adds `**` around User-B's `bold`), WYSIWYG base blame would attribute the visible `bold` to User-B — the character author. The `findFormattingOverrides` helper in `pm-position-map.ts` detects pure-formatting edits (single delimiter client + single different inner-text client) and emits an override that credits the formatter with a tooltip `"text by <B>, formatted by <A>"`. Mixed-client cases fall back to base attribution. Only applies to WYSIWYG; source mode shows exact character-level blame including the syntax chars themselves
