...
func encodeItem(enc *encoding.Encoder, item *Item, offset int, store *StructStore) {
	...
	// Mirror lib0/yjs's writer: parentSub is on the wire ONLY when neither
	// origin is set (cantCopyParentInfo). When an origin is present, the
	// parentSub is inherited from the origin's parentSub, so omitting it
	// from the wire is correct. Upstream ygo v1.1.2 wrote parentSub
	// unconditionally, which produced bytes that lib0/yjs would
	// misinterpret. Locally patched.
	if item.ParentSub != "" && origin == nil && originRight == nil {
...
func decodeItem(dec *encoding.Decoder, doc *Doc, client ClientID, clock uint64) (*Item, error) {
	...
	// lib0/yjs reads parentSub ONLY when neither origin is set
	// (`cantCopyParentInfo` in lib0). When at least one origin exists,
	// the parent is inherited from that origin's parent, and the
	// flagHasParentSub bit in `info` is effectively a no-op — lib0 does
	// not write a parentSub varstring on the wire even though the bit is
	// set. Upstream ygo v1.1.2 reads parentSub whenever the bit is set,
	// which consumes 2+ bytes of subsequent content and desyncs the
	// decoder. The visible symptom is "unknown Any tag" errors on
	// downstream byte interpretation. This is a locally patched fix —
	// upstream PR pending.
	cantCopyParentInfo := !hasOrigin && !hasRightOrigin
	if hasParentSub && cantCopyParentInfo {
	...
	// (ParentSub inheritance is deferred to integration time — see the
	// inheritParentSubFromOrigin call in the apply path. Inheriting at
	// decode time was unreliable because the origin item may belong to a
	// later client group not yet in the store.)
	if item.Parent == nil {
		if origin != nil {
