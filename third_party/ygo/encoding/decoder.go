...
func (d *Decoder) readAny(depth int) (any, error) {
	...
	case 122:
		// BigInt64 — lib0 writes 8 bytes big-endian via DataView.setBigInt64.
		// Locally patched in third_party/ygo: upstream v1.1.2 lacks this case
		// and rejects the entire update, which silently desyncs r.ydoc on
		// every JS Y.Map write that contains a bigint (timestamps, IDs).
		if d.pos+8 > len(d.buf) {
			return nil, ErrUnexpectedEOF
		}
