/**
 * Binding for text/tsx: supports Preview + Source modes.
 * Same as JsxBinding but with TypeScript language for CodeMirror.
 */
import type {
  IEditorBinding,
  EditorMode,
...
export class TsxBinding implements IEditorBinding {
  ...
  async mount(container: HTMLElement, mode: EditorMode, options: MountOptions, collab?: CollaborationContext | null): Promise<void> {
    if (!this.supportedModes.includes(mode)) throw new Error(`TsxBinding does not support mode: ${mode}`);
    ...
    this._sourceEditor.onUpdate((content) => { this._contentCallbacks.forEach(cb => cb(content)); });
  ...
  setContent(text: string): void {
    ...
    if (this._collab) {
      this._collab.ydoc.transact(() => { if (this._collab!.sharedText.length > 0) { this._collab!.sharedText.delete(0, this._collab!.sharedText.length); }
